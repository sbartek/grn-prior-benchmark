...
def make_embedder(spec, data, train_idx, device, seed, epochs, X_input):
    ...
    # AE trains only on train donors; val is a slice of train for early stopping
    Xtr_all = X_input.copy()
    # restrict training rows: pass full X but train_ae splits by index -> build a sub-run
...
def run_cv(data, spec, task_key, condition, device, seeds=(0, 1), n_splits=5, epochs=250):
    """Return list of per-(seed) macro-F1 means for one model x condition x task."""
    ...
    for seed in seeds:
        rng = np.random.default_rng(seed)
        # input features for this condition
